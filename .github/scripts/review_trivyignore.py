...
def parse_trivyignore(file_path: str) -> List[IgnoredVulnerability]:
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        ghsa_match = re.match(r"^(GHSA-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4})$", stripped)
...
def check_for_patches(vuln: IgnoredVulnerability) -> Dict:
    ...
    if advisory:
        ...
        # Check if patched versions exist
            ...
            print("  ⏳ No patches available yet")
    ...
    if no_patch:
        ...
        for r in no_patch:
            ...
            if advisory:
                url = advisory.url
                updated = (
                    advisory.updated_at[:10]
                    if advisory.updated_at and len(advisory.updated_at) >= 10
                    else advisory.updated_at or "Unknown"
                )
    ...
    for r in results:
        ...
        if advisory:
            ...
            published = (
                ...
                if advisory.published_at and len(advisory.published_at) >= 10
                else advisory.published_at or "Unknown"
            )
            updated = (
                advisory.updated_at[:10]
                if advisory.updated_at and len(advisory.updated_at) >= 10
